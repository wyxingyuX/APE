...
namespace nodesampler {
  ...
  class NodeSampler {
   ...
   public:
    ...
    NodeSampler(const double *dist, const int dist_size, const double neg_sampling_power = NEG_SAMPLING_POWER,
                const uint64 rand_seed = 0, const int *remap_list = NULL):
        neg_table(NULL),
      ...
      if (remap_list != NULL) {
        remap = new int[dist_size];
        for (int i = 0; i < dist_size; ++i)
          remap[i] = remap_list[i];
      }
    ...
    inline int sample() {
      if (remap == NULL)
        ...
        return remap[neg_table[Rand(seed)]];
    }
    ...
    inline void sample_batch(int n, int *result) {
      if (remap == NULL) {
        ...
        for (size_t i = 0; i < n; i++)
          result[i] = remap[neg_table[Rand(seed)]];
      }
    }
  };
}  // namespace nodesampler
