...
cdef extern from "nodesampler.cpp" namespace "nodesampler":
  cdef cppclass NodeSampler:
    NodeSampler(double *, int, double, unsigned long long, int *) except +
    int sample()
...
cdef class MultinomialSampler:
  ...
  def __cinit__(self, np.ndarray[double, ndim=1, mode="c"] input not None, int dist_size,
                ...
                np.ndarray[int, ndim=1, mode="c"] remap = None):
    if remap is None:
      self.thisptr = new NodeSampler(&input[0], dist_size, neg_sampling_power, rand_seed, NULL)
    else:
      assert input.size == remap.size, [input.size, remap.size]
      self.thisptr = new NodeSampler(&input[0], dist_size, neg_sampling_power, rand_seed, &remap[0])
